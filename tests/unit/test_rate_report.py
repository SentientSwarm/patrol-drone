...
def _sample(
    topic: str,
    counts: int | tuple[int, int],
    rate_stamps: list[int],
    sim_stamps: list[int] | None = None,
) -> TopicSample:
    # counts is (info_count, reader_rows), or a single int when they are equal (the clean case).
    info, rows = counts if isinstance(counts, tuple) else (counts, counts)
    return TopicSample(
        topic,
        info_count=info,
        reader_rows=rows,
...
def test_clean_bag_passes_consistency() -> None:
    ...
    ok, reasons = consistency_verdict([_sample(_CAMERA, 303, stamps, stamps)])
    assert ok is True
    assert reasons == []
...
def test_sim_dup_factor_over_threshold_fails_consistency() -> None:
    ...
    sample = _sample(_CAMERA, 300, _ramp(300, 30.0), _ramp(150, 15.0, dup=2))
...
def test_sim_dup_factor_within_threshold_passes() -> None:
    ...
    ok, _ = consistency_verdict([_sample(_CAMERA, 151, sim, sim)])
    assert ok is True
...
def test_no_sim_stamps_skips_dup_guard() -> None:
    ...
    sample = _sample(_STATE, 200, _ramp(200, 10.0))
...
def test_evaluate_true_rates(camera_hz: float, state_hz: float, passes: bool) -> None:
    samples = [
        _sample(_CAMERA, 150, _ramp(150, camera_hz)),
        _sample(_STATE, 100, _ramp(100, state_hz)),
    ]
