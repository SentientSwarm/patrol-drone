...
- SWM-31 — Re-measure the SITL runtime/flakiness budget (OQ-5): replace the provisional ≤8 min/
  ...
  timings; if the manual exit-checklist runs haven't produced them yet, build the measurement harness
  and flag the numbers as the blocking input rather than inventing them.
...
- MERGE WALL: the `require-pr` ruleset makes the automated reviewer's approval structurally mandatory
  ...
  findings with a PR COMMENT, never a commit. (Get a ruleset bypass actor set before merge — that's on
  the human, note it in the PR body.)
