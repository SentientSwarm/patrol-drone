...
- [x] **AC-6 (exit-checklist #8):** *Given* a recorded mission bag, *when* it is opened in Foxglove, *then* the camera feed, mission state, and 3D pose history render with expected panels populated. (M8 Exit) — **PASS** (e2e_check step 5, human visual witness 2026-07-03: camera feed, mission state, and 3D pose history panels all populate under [`analysis/foxglove/patrol_layout.json`](../../../analysis/foxglove/patrol_layout.json)).
...
- [x] **AC-8 (supports integrative #1):** *Given* a full multi-checkpoint patrol launched via `mission_patrol.launch.py`, *when* it completes, *then* the bag it produced is the artifact that satisfies AC-1 through AC-6 end-to-end (record → upload → manifest → replay → Foxglove). (M8 Exit; checklist #1 integrative) — **PASS** (witnessed 2026-07-03 on ONE bag, `patrol_20260703T070221Z_20260703_070221`, recorded at RTF ≈ 1 via `PX4_SIM_SPEED_FACTOR=0.33`; all five stages with no manual stitching — see [`analysis/e2e_check.md`](../../../analysis/e2e_check.md)).
