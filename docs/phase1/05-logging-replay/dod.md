...
- [x] **AC-3 (exit-checklist #6):** *Given* a mission ends, *when* the upload daemon runs, *then* the bag is uploaded to the DGX automatically (target within 30s of mission end). (M8 Exit) — **PASS** ([`analysis/e2e_check.md`](../../../analysis/e2e_check.md) step 2, 2026-07-03; re-verified 2026-07-25 on the same artifact after the wrap-up changes: **2 s** to the stand-in target, `.uploaded` receipt listing both bag + sidecar).
- [x] **AC-4 (exit-checklist #6):** *Given* a bag uploaded to the DGX, *when* ingestion indexes it, *then* it appears in the manifest with mission, time, duration, topics, and metadata. (M8 Exit) — **PASS** (e2e_check step 3, 2026-07-03; re-verified 2026-07-25: `manifest_query` returns `mission=20260703T070221Z 169s 34 topics`, matching `ros2 bag info`'s 168.81 s / 34 topics — facts derived from the bag, not the sidecar, per the §3.4 dumb-producer invariant).
- [x] **AC-5 (exit-checklist #7):** *Given* a checked-in reference bag, *when* the replay regression test runs in CI, *then* it replays the bag, subscribes to expected topics, and asserts they appear at expected rates — passing in CI. (M8 Exit) — **PASS** (the required `replay-regression` lane, green on the pinned GHCR image per SWM-84; e2e_check step 4 re-verified 2026-07-25 against the live bag: consistency guard OK, every asserted topic at rate, exit 0).
- [x] **AC-6 (exit-checklist #8):** *Given* a recorded mission bag, *when* it is opened in Foxglove, *then* the camera feed, mission state, and 3D pose history render with expected panels populated. (M8 Exit) — **PASS** (e2e_check step 5, a human visual witness twice on independent artifacts: 2026-07-03 on `patrol_20260703T070221Z_20260703_070221`, and again 2026-07-26 on `patrol_20260726T110419Z_20260726_110419` — the current evidence; camera feed, mission state, and 3D pose history panels all populate under [`analysis/foxglove/patrol_layout.json`](../../../analysis/foxglove/patrol_layout.json)).
- [x] **AC-7 (exit-checklist #11, consumer side):** *Given* the perception node publishes `patrol_interfaces/msg/CheckpointCapture` on `/patrol/checkpoint_capture`, *when* a mission runs, *then* the bag pipeline records that topic — demonstrating the message is consumed by the bag pipeline as well as the perception node. (M7 topic list; checklist #11) — **Record side PASS** (SITL 2026-06-26: recorder subscribed to `/patrol/checkpoint_capture` with the correct compiled type; topic present in `ros2 bag info`). **Non-zero count — CLOSED.** It was 0 at M7 because a full patrol hit [ADR-0010](../../decisions/0010-m6-capture-verified-dwell-stability-deferred.md)'s 02/M4 dwell-stability gap (the drone never framed each tag in the trigger window) — not an M7 defect. With [ADR-0012](../../decisions/0012-m4-dwell-pose-camera-framing-fix.md)'s dwell-pose camera-framing fix landed, the unchanged recorder records the captures the now-correct dwell pose makes possible: **Count 9** on the ADR-0012 patrol, **Count 3** on the 2026-07-03 witness bag (re-confirmed 2026-07-25), with the correct compiled `patrol_interfaces/msg/CheckpointCapture` type. See [ADR-0011](../../decisions/0011-m7-recording-verified-checkpoint-capture-count-deferred.md). *(Both counts satisfy AC-7, which asks only that the recorder capture the topic with a non-zero count — they are **message** counts, and a message count is not the capture arithmetic. The capture arithmetic is the route: 3 tag-bearing checkpoints against 4 waypoints (the 4th is a deliberately tagless inline ENU overlook), so the expectation is **3 captures, a 3/3 rate** — derived from `sim/config/checkpoints.yaml` / the tag models / `patrol_world.sdf` / `patrol_mission.yaml`, and pinned by `tests/unit/test_patrol_capture_expectation.py`. The two measures are demonstrably not interchangeable: ADR-0012's patrol read **Count 9** while writing exactly **three** on-disk captures (`000_cp_north` / `001_cp_east` / `002_cp_south`), so message count ≠ capture count on that run. Why that bag read 9 is **not established here** — that ADR does record stale-publisher contamination from overlapping local runs in the same session, though it documents it for `/patrol/mission_state`, not for this topic. Read the capture rate off the route and the on-disk set, never off `ros2 bag info`. See F-01 in [`docs/phase1-wrapup-findings.md`](../../phase1-wrapup-findings.md).)*
- [x] **AC-8 (supports integrative #1):** *Given* a full multi-checkpoint patrol launched via `mission_patrol.launch.py`, *when* it completes, *then* the bag it produced is the artifact that satisfies AC-1 through AC-6 end-to-end (record → upload → manifest → replay → Foxglove). (M8 Exit; checklist #1 integrative) — **PASS** (witnessed twice on independent artifacts: 2026-07-03 on `patrol_20260703T070221Z_20260703_070221`, and again 2026-07-26 on `patrol_20260726T110419Z_20260726_110419` — the current evidence. Both recorded at RTF ≈ 1 via `PX4_SIM_SPEED_FACTOR=0.33`; each carries all five stages on one bag with no manual stitching — see [`analysis/e2e_check.md`](../../../analysis/e2e_check.md)).
