...
- **OQ-5 — Resolved (provisional budget); measurement harness landed (SWM-31, 2026-07-24).** Two canonical SITL scenarios (basic + reduced patrol) plus, added at close-out, a low-battery abort and a `/fmu/*` bridge-surface scenario (SWM-15/32), run in the nightly SITL tier. Provisional budget ≤8 min/scenario including spin-up; quarantine-not-expand on a >1-in-5 flake rate or a >2x budget overrun; never a required per-PR check. The thresholds are now **data**, externalized to `tests/integration/sitl_budget.yaml`, and the nightly emits JUnit which `scripts/measure_sitl_budget.py` turns into a per-scenario measured wall-clock + flake verdict (job summary, non-gating). The runtime figure stays **provisional until the measured wall-clock from 01's landed SITL replaces it** — that measurement is the blocking input from the human-owned nightly runs and is **not invented** here (fold the observed max into the config + flip `provisional:false` once stable).
...
| # | Task | Files Touched | Component | Layer | Size | Dependencies | Linear |
|---|------|---------------|-----------|-------|------|-------------|--------|
| MZ.1 | SITL runtime/flakiness budget re-measure (OQ-5) — **harness landed (SWM-31):** `scripts/measure_sitl_budget.py` + `tests/integration/sitl_budget.yaml` + nightly JUnit emit/measure. Measured figure pending human-owned live runs (blocking input, not invented) | `.github/workflows/sitl-nightly.yml`, `scripts/measure_sitl_budget.py`, `tests/integration/sitl_budget.yaml`, design §2/OQ-5 | Test suites | Test | M | M2 | *harness done; measured value TBD* |
...
- **OQ-5 SITL-runtime budget re-measure** — the measurement harness landed at close-out (SWM-31: `scripts/measure_sitl_budget.py` + `tests/integration/sitl_budget.yaml` + nightly JUnit emit/measure). The provisional `≤8 min/scenario` is replaced by a measured wall-clock **only once the human-owned live runs feed the harness** — that number is the blocking input, not invented (MZ.1).
