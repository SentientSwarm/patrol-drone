...
- [ ] **AC-4** — GIVEN the `MissionStateMachine` class, WHEN the unit suite runs, THEN it passes with >80% coverage on the mission state machine and completes in <5 s without ROS/Gazebo/PX4. *(M3 Exit; exit-checklist item 3; ADR-0002 enforces ≥85% as the CI floor)*
