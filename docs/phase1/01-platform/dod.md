...
- **README command-budget allocation** · how the ≤20-command budget is split between platform bring-up and the per-docset run steps siblings append · integrative item 10 spans all docsets; platform owns the spine but the total must stay under budget. **→ Resolved (Phase 1 close-out):** finalized in the README "Setup → running mission: the command budget" table — platform spine 8 ≤ 12, total-to-running-patrol 9 ≤ 20 (AC-7). This closes PRD/Design OQ-6.
...
- **As-shipped deltas (design-time → landed), all compatible with §5–6:**
  - PX4 pin (OQ-3) settled at **v1.17.0** (`px4_msgs` release/1.17), proven by the green in-workspace
    ...
    (e.g. `vehicle_local_position_v1`); bare offboard inputs unchanged — see ADR/README M2 note.
  - The Micro XRCE-DDS Agent is **built from source** at the pinned eProsima tag (no Jazzy apt
    package) — ADR-0007; both host and `sim` container build the same pin.
- **Still provisional by design (unchanged, human-confirmed 2026-06-03):** OQ-7 (checkpoint frame
  compatibility, owned by 03) and OQ-8 (`CheckpointCapture` landing-slot, owned by 04) — 01 owns
  neither; the `gz_x500` baseline and `patrol_interfaces` shell remain compatible.
- **Human-owned exit verification** (run separately, not gating this reconciliation): AC-1 (QGC
  60 s hover), AC-4/AC-5 (`docker compose` build + in-container `colcon build`) are confirmed by
  the live/manual exit-checklist pass.
