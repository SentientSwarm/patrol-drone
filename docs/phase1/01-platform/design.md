...
- **MZ (terminal — consolidation & deferred backlog):** no new platform capability; absorbs work surfaced during M1–M2 that isn't blocking. Seeded with: **OQ-3** (pin the exact PX4 v1.16.x tag + matching `px4_msgs` branch once the M1–M2 integration spike settles it — retrofit the manifest + docs), **OQ-6** (finalize the ≤20-command README budget once 02–05 contribute their run-step counts), e2e/integration test expansion, and a final documentation+test consolidation. **Exit:** MZ reviewed and cleared or items explicitly punted to Phase 2.

§6.3 Layered Delivery: M1 is a shippable demo on its own (flying SITL drone). Scope-shedding order within M2: README polish (T2.8) → `dev` ergonomics (T2.4) → `gpu` profile. **Hard floors that cannot be shed:** green `colcon build` (PLAT-4, exit item 9) and the live `/fmu/*` bridge (PLAT-2). Parallel: vendoring+shells (T2.1/T2.2) parallelize with `sim` entrypoint (T2.3) once the M1 spike fixes the PX4 tag; OQ-7/OQ-8 resolve out-of-band at the joint `/drive` review and do not block platform tasks (for OQ-7, 01 owns no frame convention — it only ships the vanilla `gz_x500` baseline that must not contradict 03's chosen frame; for OQ-8, 01 only provides the empty `patrol_interfaces` slot). §6.4 Definition of Done (every milestone) includes the shippable-demo gate **and a documentation true-up** (PRD/Design/DoD/README reconciled against what was actually built that milestone). *(Full §6.1/§6.2 tables unchanged from v0.1.0 except §6.3 OQ-7 phrasing; canonical text on disk.)*
