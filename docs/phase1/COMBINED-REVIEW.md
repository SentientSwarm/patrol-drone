...
| 2 | Capture trigger (02↔04) | ⚠️ **Superseded 2026-06-21 (PR #8):** 02 emits an atomic `/patrol/dwell` event (`std_msgs/Int32`, dwelled index, one per DWELL entry, 02-owned); 04 captures exactly once per event. *(Originally: 02 publishes the index on `/patrol/current_waypoint` and 04 correlates it with the `DWELL` state — that correlation carried a cross-topic delivery race surfaced across Hermes PR #8 reviews R6–R11; the atomic event removes it. See 02 design OQ-7.)* | ✅ default confirmed; revised (see note) |
...
| 4 | `/patrol/*` mission topic types | `std_msgs`: `mission_state`=String, `current_waypoint`=Int32, `dwell`=Int32 (atomic OQ-7 capture event, added 2026-06-21), `abort`=Bool (typed states live in the state-machine code) | ✅ default confirmed |
...
- Decision: Capture-trigger contract (02↔04) — recommended default 02 emits the observable `DWELL` state + index on the 02-owned `/patrol/current_waypoint`; 04 captures exactly once per DWELL (no new topic owned by 02) — affects 02-mission-control, 04-perception. Confirm explicit-DWELL-signal vs 04-infers-from-AprilTag-in-view+dwell. **⚠️ Superseded 2026-06-21 (PR #8):** the trigger is now the atomic, 02-owned `/patrol/dwell` topic (`std_msgs/Int32`, dwelled index, one per DWELL entry) instead of the `DWELL` + `current_waypoint` correlation — the correlation carried a cross-topic delivery race (Hermes PR #8 R6–R11). See decision #2 above and 02 design OQ-7.
...
- Decision: /patrol/* mission topic types — recommended default the std_msgs types resolved in 02 design OQ-3 (`/patrol/mission_state`=std_msgs/String, `/patrol/current_waypoint`=std_msgs/Int32, `/patrol/abort`=std_msgs/Bool) — affects 02 (owner-of-record), 05 (records/asserts them), 04 (binds its trigger to /patrol/current_waypoint). Reconciler pinned these into 05. **Updated 2026-06-21 (PR #8):** added the 02-owned `/patrol/dwell`=std_msgs/Int32 atomic capture event (OQ-7); 04 now binds its trigger to `/patrol/dwell` (not `/patrol/current_waypoint`), and 05 records it.
...
- ⚠️ DRIFT **/patrol/mission_state, /patrol/current_waypoint, /patrol/abort topic names + types** (owner 02-mission-control) — Names agree everywhere. TYPES drift: 02 design OQ-3 RESOLVED these to std_msgs: /patrol/mission_state=std_msgs/String, /patrol/current_waypoint=std_msgs/Int32, /patrol/abort=std_msgs/Bool. 05's design still carries these as 'TBD (02-owned)' (it correctly defers ownership to 02 but is stale on the now-resolved values, so the recorded-topics table and replay assertions do not pin the std_msgs types). 04 binds its capture trigger to /patrol/current_waypoint (a 02-owned topic) consistent with 02's ownership, but does not restate the type. Fix is to propagate 02's resolved std_msgs types into 05 (and optionally note them in 04). **⚠️ Superseded 2026-06-21 (PR #8):** 04's capture trigger is now the atomic, 02-owned `/patrol/dwell` (`std_msgs/Int32`, dwelled index) — not `/patrol/current_waypoint` — and 05 records `/patrol/dwell` alongside the other `/patrol/*` topics. See decision #2/#4 and 02 design OQ-7.
...
- **/patrol/* mission topic types** — RESOLVED by 02 design OQ-3 to std_msgs (`/patrol/mission_state`=String, `/patrol/current_waypoint`=Int32, `/patrol/abort`=Bool). Reconciler pinned these into 05 §3.3 + §4.2.5 (were 'TBD'). **Updated 2026-06-21 (PR #8):** added `/patrol/dwell`=std_msgs/Int32 (atomic OQ-7 capture event); 04 binds its trigger to the 02-owned `/patrol/dwell`. Confirm at review.
- **Capture-trigger contract (02↔04, OQ-3/OQ-7)** — **Superseded 2026-06-21 (PR #8):** 02 emits the atomic `/patrol/dwell` event (`std_msgs/Int32` index) once per DWELL entry; 04 captures once per event. *(Was: observable `DWELL` + `/patrol/current_waypoint` correlation — removed for the cross-topic race, Hermes R6–R11.)* Confirm at review.
