...
| OQ-3 | Capture trigger mechanism: 02 publishes an explicit "capture now" signal vs 04 infers a checkpoint visit from AprilTag-in-view + dwell. | PRD OQ-3 (PCAP-1) | **Resolved (design decision) → explicit trigger from 02, with tag-in-view as the gate** | §4.2.2, ADR-A. Design keys capture off 02's atomic `/patrol/dwell` capture event (a 02↔04 joint contract; `/patrol/dwell` is a 02-owned `std_msgs/Int32` carrying the dwelled waypoint index, one event per DWELL entry, per the Phase 1 README matrix + 02 design OQ-7); 04 does NOT infer the visit unilaterally. **Confirmed by 02 (2026-06-21, PR #8):** the trigger is the atomic `/patrol/dwell` event, not the earlier `DWELL` + `/patrol/current_waypoint` correlation (which carried a cross-topic race). The trigger topic remains a node parameter (§4.2.1). |
...
| OQ-6 | `apriltag_ros` (or equivalent) availability/buildability on ROS 2 Jazzy in-container. | PRD OQ-6 (PCAP-2) | **Resolved (2026-06-23, T B.1) — `ros-jazzy-apriltag-ros` 3.3.0 apt deb** | VP-1 VERIFIED: installs as a Jazzy apt deb (no source build, no `apriltag` C-lib fallback). `apriltag_msgs/msg/AprilTagDetection` carries `id`/`family`/`decision_margin` — the fields `CheckpointResolver` already consumes. |
...
| OQ-9 [INFERRED] | `cv_bridge` (Image→numpy/encoded-file) availability on Jazzy in-container, used by FrameSampler/CaptureWriter to decode `sensor_msgs/Image` and encode PNG/JPEG. | Design (implementation of PCAP-1/PCAP-5) | **Resolved (2026-06-23, T B.1) — `ros-jazzy-cv-bridge` 4.1.0 apt deb** | VP-2 VERIFIED: `cv_bridge` 4.1.0 + `image_transport` 5.1.7 present under `/opt/ros/jazzy`; no minimal-PNG-writer fallback needed. |
...
|---|-------|----------------------|--------|-------------------|
| VP-1 | An `apriltag_ros`-equivalent ROS 2 package installs/builds on Jazzy in the sim/dev container and publishes detections carrying `tag_id` (+ family, +pose/confidence where available). | `apt-cache policy ros-jazzy-apriltag-ros` in the 01 container; else source-build probe in `ros2_ws`. | **VERIFIED (2026-06-23, T B.1) — apt deb, no source build / no C-lib fallback** | `ros-jazzy-apriltag-ros` installed 3.3.0 (apt candidate 3.4.0); `apriltag_msgs/msg/AprilTagDetection` carries `id` (→`tag_id`) + `family` + `decision_margin` (→`detection_confidence`, PCAP-7), exactly what `CheckpointResolver` (T A.3) duck-types. OQ-6 resolved. |
| VP-2 [INFERRED] | `cv_bridge` + an image encoder (OpenCV PNG/JPEG) are available on Jazzy in-container to decode `sensor_msgs/Image` and write a file. | `python3 -c "import cv_bridge, cv2"` in the 01 container. | **VERIFIED (2026-06-23, T B.1) — apt deb** | `ros-jazzy-cv-bridge` installed 4.1.0 (+ `image_transport` 5.1.7) under `/opt/ros/jazzy`. OQ-9 resolved; no minimal-PNG-writer fallback needed. |
...
| VP-5 | 02 emits a checkpoint-arrival/"dwelling at checkpoint N" semantic this node can subscribe to (settled default: 02's atomic `/patrol/dwell` capture event). | Read 02 design; `ros2 topic info /patrol/dwell` (or the agreed trigger topic). | **Owned by 02 — `/patrol/dwell` (`std_msgs/Int32`, dwelled index) is the 02-owned trigger; bind at config time** | 02 DoD §5 + Phase 1 README matrix (02 owns `/patrol/dwell` + the checkpoint-arrival capture-trigger semantic); PRD OQ-3; 02 design OQ-7 (confirmed 2026-06-21, PR #8 — atomic event, not the `/patrol/current_waypoint` correlation). Trigger topic name is a node parameter (§4.2.1). |
...
(No external precondition is asserted as already-true in §4 prose without a row here. VP-1/VP-2 were the only design-blocking unverified claims; both VERIFIED 2026-06-23 at T B.1 — see rows above.)

### 3.3 Architectural Decisions

**ADR-A — Capture trigger is an explicit signal from 02, gated by tag-in-view (resolves OQ-3).**
*Decision:* The node captures when (a) 02 signals checkpoint arrival/dwell on its waypoint/arrival topic AND (b) a checkpoint AprilTag is currently in view. *Rationale:* PRD tenet "sim path == hardware path" and the 02↔04 contract in both DoDs ("mission signals a checkpoint arrival"). Pure node-inference (AprilTag-in-view + dwell timer, no 02 signal) is rejected because it duplicates mission state the orchestrator already owns and risks firing off-route. *Implication:* PCAP-1 fires on a subscription callback, not a timer; deterministic one-capture-per-visit (AC-6) is enforced by a per-visit latch (§4.2.8). Confirmed by 02 (2026-06-21, PR #8): the trigger topic is the atomic `/patrol/dwell` event (`std_msgs/Int32`, dwelled index).

**ADR-B — Latest-frame-on-trigger, bounded by a per-stream freshness window; capture pose in the world/ENU frame with explicit `frame_id` (resolves OQ-7).**
*Decision:* On trigger, sample the most recent buffered camera frame and the most recent pose; stamp the `PoseStamped.header.frame_id` with the world/ENU frame name (matching 03's `checkpoints.yaml` positions). Latest-frame is **bounded by an explicit per-stream freshness window**: a buffered detection / frame / pose older than its configured max age (measured from node-receipt time) is treated as *stale* and rejected exactly as an *absent* buffer is — skip the visit, leave the token unlatched, log a `stale_*` reason (§4.2.8 / §4.4.5). *Rationale:* Phase 1 uses ground-truth sim pose (no VIO); a checkpoint visit is a quasi-static hover, so latest-frame is sufficient and far simpler than time-synchronized message filters. The original "sampling skew is acceptable for a hover" assumption covers the *skew between two live streams*; it does **not** cover a *stalled* stream (a detector hiccup / dropped camera frames / a pose telemetry gap), where the keep-latest buffer would otherwise hand the next visit a tuple belonging to the previous visit — a false capture (reproduced 2026-06-25, Hermes review 4566902694). A stall is not skew, so the latest-frame contract is bounded, not abandoned. Tenet 5: frames are explicit, never implicit. *Implication:* No `message_filters` time-sync dependency in Phase 1 (the windows are independent per-stream age checks, not a synchronizer); the receipt-age check reuses the node clock already injected into `CaptureCoordinator` (the ROS-free testing seam), so no `header.stamp` parsing is introduced. Time-sync remains a Phase-3+ thickening if motion-blur becomes an issue.
...
| `max_detection_age_s` | `1.0` | freshness window for `/tag_detections` (ADR-B) | ADR-B |
| `max_frame_age_s` | `0.5` | freshness window for the camera frame (ADR-B) | ADR-B |
| `max_pose_age_s` | `1.0` | freshness window for the `/fmu/out` pose (ADR-B) | ADR-B |

All nine parameters are internal configuration seams whose values trace to a cross-docset interface (the OQ/VP column) or a resolved ADR — none introduces a new requirement surface beyond the PRD's FR table.

> **As-built default notes (M6).** `output_root` defaults to empty and resolves to a CWD-relative `captures/` dir at the node; the launch graph exposes an `output_root` argument (forwarded `mission_patrol` → `patrol_perception` → node) so a top-level patrol run can point captures under 05's bag/run dir for OQ-4 alignment without editing code. `world_frame` is the literal `patrol_world` (03's M5 world/ENU frame); the original "(from config)" intent is not realized because `checkpoints.yaml` carries no frame-name field — the positions there are *implicitly* world/ENU and 03/04 share the literal frame name by convention. If 03 ever renames the world frame, this default is the single seam to update (and would be the place to add a `frame_id` key to `checkpoints.yaml`). The three ADR-B freshness windows (`max_detection_age_s` 1.0, `max_frame_age_s` 0.5, `max_pose_age_s` 1.0 — node-receipt age in seconds) are set comfortably above the slowest expected inter-message gap for each stream (the camera runs ~15 Hz → ~0.067 s/frame; the detector and the PX4 pose bridge are slower), so the happy-path traversal is unaffected and only a genuine *stall* (a stream that stops delivering) trips the gate.
...
| Persistent state | Same-second `run_id` collision (fast restart / duplicated node within one wall-clock second) overwrites prior artifacts — `run_id` is second-precision UTC, `_index` restarts at 0, deterministic `000_<checkpoint_id>.{png,json}` paths are overwritten | second-precision `run_id` + per-writer `_index=0` + `write_bytes/write_text` overwrite | **Durability-only** (Hermes Low finding, PR #12): the published `/patrol/checkpoint_capture` message still exists (topic is the bag's source of truth) so no *wrong* capture results; the earlier instance's on-disk PNG/JSON can be clobbered. **Deferred to M7** ([ADR-0009](../../decisions/0009-m6-residual-findings-deferred-to-m7.md)) — make `run_id` collision-resistant (sub-second/UUID) and/or write with exclusive creation, applied once at the run-id seam so captures and bags share it (OQ-4) | Make `run_id` collision-resistant in M7; a normal patrol does not restart perception within one second |
| Network dependency (topic) | Camera topic silent at trigger — no frame ever buffered | No buffered frame in FrameSampler | Log `no_frame_available`; skip the visit (no malformed capture); cardinality latch NOT set so a re-trigger can retry | Auto-recovers when frames resume |
| Network dependency (topic) | Camera / detector / pose stream **stalled** — buffer present but older than its freshness window (ADR-B) | Buffered value age > `max_{frame,detection,pose}_age_s` (node-receipt age) in `CaptureCoordinator` | Log `stale_frame` / `stale_detection` / `stale_pose`; skip the visit (no stale capture for a different visit); latch NOT set so a re-trigger retries once the stream resumes | Auto-recovers when the stalled stream resumes within its window |
...
The "skip" rows all share the **latch-not-set** invariant from §4.2.8: a skipped visit is retryable, and only a *successful* capture latches the token. There are four skip *categories* — **absent buffer** (no-frame), **no-tag-in-view**, **unmapped-tag_id**, and (ADR-B) **stale-but-present buffer** (stale-frame / stale-detection / stale-pose) — and all four obey the same rule. The latch-not-set invariant is what reconciles AC-6 (deterministic, ≤1 per visit) with AC-4 (no fabricated `checkpoint_id`): a visit that cannot be honestly identified produces zero captures, not a wrong one, and is not silently swallowed (every skip logs). The stale-but-present category is the ADR-B freshness bound — a stalled stream is skipped exactly as an absent one, so a tuple from a previous visit can never be published for a later one. Complementarily, a *successful* capture **expires the detection buffer** (the coordinator clears it after latching), so a no-longer-visible tag cannot be reused on the next trigger even before its freshness window elapses. **Boundary (Phase-1 scope):** the freshness window ages each buffer from its *node-receipt* time, not the message's source `header.stamp` (ADR-B / §4.2.6 — no `header.stamp` parsing in Phase 1), so it bounds a *stalled callback* but **not** an *old-but-recently-delivered* sample (callback backlog, transport delay, or bag replay deliver a source-old frame that still reads as fresh). A reviewer probe on PR #12 confirmed this: an image with `header.stamp = (0,0)` but a fresh receipt time still published (Hermes Medium finding). This source-vs-receipt skew is **deferred to M7** — the milestone where it first bites, since replayed bags deliver source-old `header.stamp` by construction and M7 ships the replay harness to validate the fix ([ADR-0009](../../decisions/0009-m6-residual-findings-deferred-to-m7.md)); the *full* time-synchronized sampling remains Phase 3+ (§6 future work). The non-obvious M7 trap is the per-stream clock domain: the ROS-stamped camera/detection streams gate on `header.stamp`, but PX4 `VehicleLocalPosition` carries a PX4-microsecond `timestamp` on a different epoch — mixing the two clocks naively manufactures a new false-skip/false-pass, which is why this is M7 work, not a one-line tweak here. In Phase 1's ground-truth quasi-static hover the skew does not affect capture correctness.
...
| Source-`header.stamp` freshness gate (reject old-but-recently-delivered samples) | §4.4.5 boundary / [ADR-0009](../../decisions/0009-m6-residual-findings-deferred-to-m7.md) (PR #12 Medium) | M7 (replay makes it load-bearing) |
| Collision-resistant `run_id` (sub-second/UUID) so same-second restarts can't overwrite artifacts | §4.4.5 / [ADR-0009](../../decisions/0009-m6-residual-findings-deferred-to-m7.md) (PR #12 Low) | M7 (shared with bag run dir, OQ-4) |
| Full time-synchronized frame/pose sampling (`message_filters`) | ADR-B (latest-frame is sufficient Phase 1) | Phase 3+ |
...
### v0.2.1 — 2026-06-25
**M6 PR #12 residual-findings deferral note (as-built, post-approval).** PR #12 is approved/green at head `1574703` (Hermes APPROVE / Risk Low; CodeScene + CI pass); the blocking stale-buffer false-capture was fixed (per-stream `FreshnessWindows`). Two non-blocking review findings are recorded as **deferred to M7** rather than fixed on the approved PR ([ADR-0009](../../decisions/0009-m6-residual-findings-deferred-to-m7.md)):

- **Medium** — the freshness gate ages from *node-receipt* time, not source `header.stamp`, so an old-but-recently-delivered sample (backlog / transport delay / **bag replay**) reads as fresh. M7 is where this first bites (replay delivers source-old stamps by construction). The non-obvious trap (recorded in §4.4.5 and the ADR): PX4 `VehicleLocalPosition` carries a PX4-microsecond timestamp on a different clock domain than the ROS-stamped camera/detection streams, so it is not a one-line gate.
- **Low** — same-second `run_id` collisions can overwrite on-disk artifacts (durability-only; the topic remains the source of truth). M7 makes `run_id` collision-resistant, shared with the bag run dir (OQ-4).

**Sections modified:**
- §4.4.5: boundary paragraph reconciled — source-vs-receipt skew deferred to **M7** (was "Phase 3+"), with the PX4-µs clock-domain trap spelled out; added a `run_id` same-second-collision failure-mode row.
- §6.2 (M6.B Out of Scope): split the single "Phase 3+" future-work row into the M7 source-stamp gate, the M7 collision-resistant `run_id`, and the Phase-3+ full `message_filters` time-sync.

**Codebase drift:** None (documentation-only; no code change on this revision).
