...
- M4 — multi-waypoint patrol + safety. **In progress** (`phase1/m4-multi-waypoint-patrol`, off main after PR #7): `MissionStateMachine` gains WAYPOINT/DWELL/RTH/ABORT states + the 4 abort guards (external-signal + low-battery **live**; manual-takeover + timeout **scaffold** — unit-tested but never fired in SITL), full mission YAML schema + `checkpoint_id` resolution against an interim `sim/config/checkpoints.yaml`, the `/patrol/*` `std_msgs` surface (mission_state/current_waypoint/abort, OQ-3), an explicit home-waypoint RTH (no PX4 RTL, OQ-8), `ros2 launch patrol_bringup mission_patrol.launch.py` (with a resilient 05-recorder include), and nightly patrol SITL tests with mid-patrol **external-abort and low-battery-abort** assertions — both guards SITL-observed as of 2026-07-26, each asserting its own cause via `/patrol/abort_reason` (F-09) rather than by exclusion. Layer-A unit suite 100% in <1 s. **Dwell-capture fix ([ADR-0012](docs/decisions/0012-m4-dwell-pose-camera-framing-fix.md), `phase1/m4-dwell-stability`):** the ADR-0010/0011 "0 checkpoint captures" gap was **not** a WAYPOINT↔DWELL flap (already resolved) but the stand-off hover sitting at the tag's altitude while the camera is pitched ~20° down — the tag jammed at the top frame edge and apriltag never resolved it at dwell. Fixed by climbing `standoff_m*tan(camera_pitch_rad)` above the tag in `config._approach_pose` (centers the boresight); a full patrol now records **`/patrol/checkpoint_capture` Count 9 (was 0)** with clean 4-DWELL flight. No `state_machine.py` change.
...
- M6 — perception scaffolding + checkpoint capture. **In progress** (`phase1/m6-perception-capture`, off main): `patrol_interfaces/msg/CheckpointCapture` + the ROS-free capture core (config/resolver/builder/samplers/coordinator) under `patrol_perception`, the live `PerceptionNode` (off-the-shelf `apriltag_ros` detection, per-stream freshness gate, latch-only-on-success), on-disk image+sidecar persistence, and `patrol_perception.launch.py` (included by 02's patrol). Layer-A unit suite green; SITL capture (AC-1/2/4/6) is nightly/manual.
- M7 — rosbag2 logging, MCAP format, full sensor capture. **In progress** (`phase1/m7-mcap-recording`, off the M6 tip): the `patrol_logging` package — a ROS-free recorder core (`recorder.py`: `bag_name` → `patrol_<missionId>_<timestamp>`, `build_record_argv` running `ros2 bag record --storage mcap` over a broad set, and the JSON `<bag>.meta.json` sidecar, OQ-10) + `record.launch.py` (an `ExecuteProcess` recorder that launch SIGINT-finalizes; included by 02's patrol, now `record:=true` by default) + `config/recorded_topics.yaml` (regex `/fmu/out/.*` absorbs PX4 v1.17 `_v1` churn, plus explicit `/patrol/*`, `/tag_detections`, compressed camera, `/tf`) + the `analysis/foxglove/` load-and-render note. Layer-A `test_recorder.py` green in <1 s. **SITL-verified 2026-06-26** ([ADR-0011](docs/decisions/0011-m7-recording-verified-checkpoint-capture-count-deferred.md)): a full patrol produced `patrol_patrol_20260626_080740` (98.5 MiB / 142 s / 76,780 msgs, MCAP) — **AC-1/AC-2 PASS**; **AC-7 record-side PASS** (recorder subscribes/records `/patrol/checkpoint_capture`). Its non-zero count was deferred to the 02/M4 fix and is now **closed**: with [ADR-0012](docs/decisions/0012-m4-dwell-pose-camera-framing-fix.md)'s dwell-pose camera-framing fix landed, a full patrol records **`/patrol/checkpoint_capture` Count 9 (was 0)** — confirmed end-to-end with no M7 change (the unchanged recorder records the captures the now-correct dwell pose makes possible). Upload daemon / DGX ingest / manifest / replay regression test / Foxglove auto-load are **M8**, not here.
- M8 — replay pipeline: bag → DGX → Foxglove. **In progress** (`phase1/m8-replay-pipeline`, off main): the dumb-producer/smart-ingestion pipeline around the M7 bag — `analysis/upload_daemon/` (`Transport` rsync-over-SSH default + S3 parity stub, OQ-8; `UploadDaemon` watch-dir + bag-complete guard + retry/backoff, ≤30 s, LR-3/AC-3), `docker/ingest/` (`ManifestStore` SQLite OQ-3 + `IngestService` deriving duration/topics **from the bag not the sidecar** per the §3.4 dumb-producer invariant + `bag_reader` parsing `ros2 bag info` + `manifest_query` CLI + ingest Dockerfile, LR-4/AC-4), and `tests/replay/` (the ROS-free `replay_assertions` comparator [presence + ±40% rate, OQ-5] + `test_replay_regression.py` playing a checked-in **Git LFS** reference bag [OQ-4; a 4.6 MiB / 20 s trim of a real M7 patrol via `make_reference_bag.py`, anchored on the first checkpoint capture so all asserted topics are non-zero] with a deliberate dropped-topic self-check, LR-5/AC-5, ≤90 s OQ-6). A new required `replay-regression` CI lane (ROS image, `lfs:true` checkout, MCAP plugin) runs the replay + the upload→ingest→manifest **stand-in** integration test (OQ-7, no real DGX). Plus **SWM-83** (run_id path-hygiene carried from M7 PR #14: reject `..`/separators/absolute at `recorder.resolve_run_id` + a defensive reject in `CaptureWriter`). Layer-A unit suite 577 green / 98.65% cov; the stand-in + replay tiers verified locally (replay green vs the real LFS bag, 20.7 s). **Record-path finalize fix (`record.launch.py`/`recorder.py`/`run_patrol_world_sitl.sh`):** the mission launch now runs in its own process group and the runner group-signals it (SIGINT→SIGTERM) waiting `FINALIZE_WAIT=90 s`, with the `<bag>.meta.json` sidecar staged at record-start (`<bag>.sidecar-inputs.json`) and finalized by the runner — so both `metadata.yaml` and the sidecar survive teardown regardless of how `ros2 launch` dies (closes ADR-0013's deferred item). **AC-6 + AC-8 WITNESSED 2026-07-03** (`analysis/e2e_check.md`): one bag `patrol_20260703T070221Z_20260703_070221` (recorded at RTF ≈ 1 via `PX4_SIM_SPEED_FACTOR=0.33`, camera 15.16 Hz) carried through record→upload→ingest→manifest→replay→Foxglove — `verify_live_bag.py` exit 0, manifest `169 s / 34 topics` matches `ros2 bag info` (dumb-producer), all panels render; independently re-witnessed 2026-07-26 on `patrol_20260726T110419Z_20260726_110419`, the current AC-6/AC-8 evidence. `checkpoint_capture` Count 3 is the **correct** count, not a gap: the route has 4 waypoints but only **3 checkpoints** (the 4th is a deliberately tagless inline ENU overlook that exercises the inline-waypoint path), so the capture rate is 3/3. The earlier "3 of 4 checkpoints / perception-timing follow-up" reading was a false finding (F-01), closed after investigation and pinned by `tests/unit/test_patrol_capture_expectation.py`.
